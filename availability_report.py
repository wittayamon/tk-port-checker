...
from monitoring_state import EVENT_DOWN, EVENT_RECOVERED, format_duration
...
SUMMARY_CSV_COLUMNS = (
    "device_name", "host", "port", "period_start", "period_end",
    "availability_percent", "coverage_percent", "period_duration_seconds", "known_duration_seconds",
...
def _normalize_events(events: Iterable[EventRecord]) -> list[_TimedEvent]:
    ...
    for sequence, event in enumerate(events):
        if str(event.event_type).upper() not in (EVENT_DOWN, EVENT_RECOVERED):
            continue
...
def _outages_for_target(
    events: Sequence[_TimedEvent], period_start: datetime, period_end: datetime,
    device_name: str, host: str, port: int,
) -> list[OutageDetail]:
    outages = []
    state = "UNKNOWN"
    down_at = None
...
def calculate_availability(
    ...
    for key in keys:
        ...
        name = names.get(key, "")
        known, downtime = reconstruct_state_intervals(target_events, start, end)
        ...
        outages = _outages_for_target(target_events, start, end, name, host, port)
...
def filter_report_rows(rows: Iterable[AvailabilityRow], search: str = "") -> list[AvailabilityRow]:
    needle = str(search or "").strip().casefold()
    if not needle:
        return list(rows)
    return [row for row in rows if needle in row.display_name.casefold() or needle in row.host.casefold()]
