...
## Health Watchdog and Incident History (v1.13 development)

The application includes one bounded internal watchdog, enabled by default, that runs every 15 seconds through the existing Tk lifecycle. It observes Auto Refresh orchestration, notification worker/scheduler state, notification queue backlog, Event/Notification DB accessibility, maintenance scheduling, tray availability, and configuration health. A monitored target being TCP OFFLINE is not an application-health failure, and intentionally disabled Auto Refresh or providers do not create incidents.

The watchdog reports a stale Auto Refresh cycle after `max(3 × interval, 60 seconds)`, warns when the notification queue reaches 75% capacity, and records meaningful worker/database failures. Persistent Health Incident History is stored in a separate `health_incidents` table in the existing writable `events.db`; it is independent from TCP Event History and Notification Delivery History. Incidents have INFO/WARNING/ERROR severity, OPEN/RESOLVED status, occurrence counts, first/last-seen times and safe recovery results. Identical open conditions are fingerprint-deduplicated, resolved rows can be filtered or cleared, and the newest 5,000 resolved rows are retained. Open incidents are never pruned.

Application Settings includes **Internal watchdog** (enabled by default), **Windows health notifications** (disabled by default), and **Safe automatic recovery** (enabled by default). Health notifications use only the existing Windows notification path and fire on first occurrence, severity escalation, or resolution, never every watchdog tick. Automatic recovery is bounded to two attempts per condition with a 60-second cooldown. The notification worker recovery preserves durable SQLite delivery state and does not blindly requeue jobs. Database recovery is non-destructive: no tables or data are deleted or recreated automatically. Maintenance reconciliation uses the existing canonical expiry path. If the app is hidden and tray access becomes unavailable, the main window is restored on Tk's main thread.

Health / Diagnostics shows watchdog state, aggregate open warning/error counts, last incident time, and a Health Incident History button. Diagnostics exports aggregate incident fields only; they contain no incident details, endpoint URLs, credentials, target inventory or payloads. Health preferences are portable Settings Backup fields; incident rows, recovery counters and watchdog runtime state are not portable. During intentional shutdown the watchdog stops before workers so shutdown does not create false incidents.
